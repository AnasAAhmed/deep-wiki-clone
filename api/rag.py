...
9. For pipe characters (|) in text, write them directly without escaping them"""
    ...
    def prepare_retriever(self, repo_url_or_path: str, type: str = "github", access_token: str = None,
        ...
        try:
            # Ensure database directory exists for FAISS retriever
            # FAISSRetriever uses SQLite internally, need to ensure directory exists
            from adalflow.utils import get_adalflow_default_root_path
            root_path = get_adalflow_default_root_path()
            index_dir = os.path.join(root_path, "indexes")
            ...
            try:
                ...
                if not os.access(index_dir, os.W_OK):
                    logger.warning(f"Index directory exists but may not be writable: {index_dir}")
                    # Try to create a test file to verify write permissions
                    test_file = os.path.join(index_dir, ".test_write")
                    try:
                        ...
                        logger.error(f"Directory write test failed: {test_err}")
                        raise PermissionError(f"Cannot write to index directory: {index_dir}")
                logger.info(f"Ensured index directory exists and is writable: {index_dir}")
            except Exception as dir_err:
                logger.error(f"Failed to create/verify index directory: {dir_err}")
                raise
            ...
            # Get retriever config and add index path if not present
            retriever_config = configs["retriever"].copy()
            if "index_path" not in retriever_config:
                # Create a unique index path per repository
                # Extract repo name from repo_url_or_path
                repo_name = 'default'
                if self.repo_url_or_path:
                    if '/' in self.repo_url_or_path:
                        # Extract from URL or path
                        parts = self.repo_url_or_path.rstrip('/').split('/')
                        if len(parts) >= 2:
                            # Format: owner/repo
                            repo_name = f"{parts[-2]}_{parts[-1]}".replace('.git', '')
                        else:
                            repo_name = parts[-1].replace('.git', '')
                    else:
                        repo_name = self.repo_url_or_path.replace('.git', '')
            ...
            if "unable to open database file" in error_msg.lower():
                logger.error("SQLite database file error detected. This may be due to:")
                logger.error("  1. Missing directory permissions")
                logger.error("  2. Disk space issues")
                logger.error("  3. File system issues")
                logger.error("  4. Directory not existing")
                
                # Try to get more context
                from adalflow.utils import get_adalflow_default_root_path
                root_path = get_adalflow_default_root_path()
                index_dir = os.path.join(root_path, "indexes")
                logger.error(f"Index directory should be: {index_dir}")
                logger.error(f"Directory exists: {os.path.exists(index_dir)}")
                if os.path.exists(index_dir):
                    logger.error(f"Directory writable: {os.access(index_dir, os.W_OK)}")
