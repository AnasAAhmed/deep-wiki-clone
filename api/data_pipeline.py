...
class DatabaseManager:
    ...
    def prepare_db_index(self, embedder_type: str = None, is_ollama_embedder: bool = None, 
        ...
        if embedder_type is None and is_ollama_embedder is not None:
            embedder_type = 'ollama' if is_ollama_embedder else None
        # Determine current embedder type
        from api.config import get_embedder_type
        if embedder_type is None:
            current_embedder_type = get_embedder_type()
        else:
            current_embedder_type = embedder_type
        
        logger.info(f"Current embedder type: {current_embedder_type}")
        ...
        if self.repo_paths and os.path.exists(self.repo_paths["save_db_file"]):
            ...
            try:
                ...
                # Check if database embedder type matches current embedder type
                # Try to detect saved embedder type from database transformer
                should_regenerate = False
                try:
                    # Get transformer from database to check embedder type
                    transformer = self.db.get_transformer(key="split_and_embed")
                    if transformer:
                        # Check if it's a ToEmbeddings component
                        if hasattr(transformer, '_components') and 'embedder' in transformer._components:
                            embedder_component = transformer._components['embedder']
                            if hasattr(embedder_component, '_components') and 'model_client' in embedder_component._components:
                                saved_client = embedder_component._components['model_client']
                                saved_client_name = saved_client.__class__.__name__
                                
                                # Check if saved client matches current embedder type
                                expected_client_names = {
                                    'openai': 'OpenAIClient',
                                    'google': 'GoogleEmbedderClient',
                                    'ollama': 'OllamaClient'
                                }
                                expected_client = expected_client_names.get(current_embedder_type, 'OpenAIClient')
                                
                                if saved_client_name != expected_client:
                                    logger.warning(f"Embedder type mismatch detected!")
                                    logger.warning(f"  Saved embedder: {saved_client_name}")
                                    logger.warning(f"  Current embedder type: {current_embedder_type} (expected: {expected_client})")
                                    logger.warning(f"  Regenerating database with current embedder type...")
                                    should_regenerate = True
                except Exception as check_err:
                    logger.warning(f"Could not check embedder type mismatch: {check_err}")
                    # If we can't check, try to load anyway - will fail if mismatch exists
                
                if should_regenerate:
                    logger.info("Deleting old database to regenerate with correct embedder type...")
                    try:
                        os.remove(self.repo_paths["save_db_file"])
                        logger.info("Old database deleted successfully")
                    except Exception as del_err:
                        logger.error(f"Failed to delete old database: {del_err}")
                    # Continue to create a new database below
                else:
